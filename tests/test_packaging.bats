#!/usr/bin/env bats
# platform: macOS-only -- runs pkgutil
# build_pkg.sh stages the Porthole.app engine + the engine CLI/templates + the CLI wrapper
# into a productbuild .pkg with a 10.9.5 floor. Uses a fake minimal Porthole.app so the test
# needs no compiled viewer (real pkgbuild/productbuild, which macOS provides).

setup() {
  ENGINE="$(cd "${BATS_TEST_DIRNAME}/.." && pwd)"
  WORK="$(mktemp -d "${TMPDIR:-/tmp}/porthole-pkg-test.XXXXXX")"
  FAKEAPP="$WORK/Porthole.app"
  mkdir -p "$FAKEAPP/Contents/MacOS" "$FAKEAPP/Contents/Resources"
  printf '#!/bin/sh\n' > "$FAKEAPP/Contents/MacOS/Porthole"; chmod 755 "$FAKEAPP/Contents/MacOS/Porthole"
  printf '<plist></plist>\n' > "$FAKEAPP/Contents/Info.plist"
}
teardown() { [ -n "$WORK" ] && rm -rf "$WORK"; }

@test "build_pkg stages the app, the engine CLI, and the wrapper into the payload" {
  run sh "$ENGINE/packaging/macos/build_pkg.sh" 9.9.9 "$WORK/out.pkg" "$FAKEAPP"
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ -f "$WORK/out.pkg" ]
  d="$WORK/expand"; pkgutil --expand "$WORK/out.pkg" "$d"
  files="$(lsbom "$d/porthole-component.pkg/Bom")"
  echo "$files" | grep -q 'Applications/Porthole.app/Contents/MacOS/Porthole'
  echo "$files" | grep -q 'Applications/Porthole.app/Contents/Resources/engine/bin/porthole'
  echo "$files" | grep -q 'Applications/Porthole.app/Contents/Resources/engine/bin/generate-viewer'
  echo "$files" | grep -q 'Applications/Porthole.app/Contents/Resources/engine/templates/launcher.tmpl'
  echo "$files" | grep -q 'usr/local/mavergreen/porthole/bin/porthole'
  echo "$files" | grep -q 'usr/local/mavergreen/porthole/mavergreen.plist'
  if echo "$files" | grep -q 'usr/local/bin/'; then false; fi
}

@test "the pkg declares a 10.9.5 minimum" {
  sh "$ENGINE/packaging/macos/build_pkg.sh" 9.9.9 "$WORK/out.pkg" "$FAKEAPP" >/dev/null
  d="$WORK/expand"; pkgutil --expand "$WORK/out.pkg" "$d"
  grep -q 'os-version min="10.9.5"' "$d/Distribution"
  [ "$(sed -n 's/.*<line choice="\([^"]*\)".*/\1/p' "$d/Distribution" | grep -v '^default$' | head -1)" = dev.mavergreen.base ]
}

@test "build_pkg stages engine/bin/s6-ipcserver when transport binaries are provided" {
  command -v pkgbuild >/dev/null 2>&1 || skip "pkgbuild not available"
  command -v pkgutil >/dev/null 2>&1 || skip "pkgutil not available"
  tdir="$WORK/transport"; mkdir -p "$tdir"
  for b in s6-ipcserver s6-ipcserver-socketbinder s6-ipcserverd; do
    printf 'fake\n' > "$tdir/$b"; chmod +x "$tdir/$b"
  done
  PORTHOLE_TRANSPORT_DIR="$tdir" run sh "$ENGINE/packaging/macos/build_pkg.sh" \
      9.9.9 "$WORK/out-transport.pkg" "$FAKEAPP"
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  d="$WORK/expand-transport"; pkgutil --expand "$WORK/out-transport.pkg" "$d"
  files="$(lsbom "$d/porthole-component.pkg/Bom")"
  echo "$files" | grep -q 'engine/bin/s6-ipcserver' \
    || { echo "transport not in payload: $files"; return 1; }
  for _b in s6-ipcserver-socketbinder s6-ipcserverd; do
    echo "$files" | grep -q "engine/bin/$_b" \
      || { echo "missing engine/bin/$_b in payload"; return 1; }
  done
}

# Each materialized app carries its own copy of the engine, so a Porthole upgrade that left them
# alone would leave every app running the old one. The pkg's postinstall re-materializes every
# installed preset against the engine it just installed.
@test "postinstall re-materializes every installed preset with the new engine" {
  command -v pkgutil >/dev/null 2>&1 || skip "pkgutil not available"
  sh "$ENGINE/packaging/macos/build_pkg.sh" 9.9.9 "$WORK/out.pkg" "$FAKEAPP" >/dev/null
  d="$WORK/expand"; pkgutil --expand "$WORK/out.pkg" "$d"
  T="$WORK/target"; mkdir -p "$T"
  (cd "$T" && gzip -dc "$d/porthole-component.pkg/Payload" | cpio -id --quiet)
  (cd "$T" && gzip -dc "$d/mavergreen-base.pkg/Payload" | cpio -id --quiet)
  P="$T/usr/local/mavergreen/thunderbird-preset/share/porthole/presets"; mkdir -p "$P"
  cp "$ENGINE/examples/thunderbird.conf" "$P/"
  mkdir -p "$T/Applications/Linux Thunderbird.app/Contents/MacOS"
  printf 'stale\n' > "$T/Applications/Linux Thunderbird.app/Contents/MacOS/Porthole"

  run sh "$d/porthole-component.pkg/Scripts/postinstall" "$WORK/out.pkg" "$T" "$T" "$T"
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  cmp -s "$FAKEAPP/Contents/MacOS/Porthole" "$T/Applications/Linux Thunderbird.app/Contents/MacOS/Porthole"
  [ -f "$T/Applications/Linux Thunderbird.app/Contents/Resources/thunderbird.container" ]
}

@test "postinstall with no presets installed does nothing and succeeds" {
  command -v pkgutil >/dev/null 2>&1 || skip "pkgutil not available"
  sh "$ENGINE/packaging/macos/build_pkg.sh" 9.9.9 "$WORK/out.pkg" "$FAKEAPP" >/dev/null
  d="$WORK/expand"; pkgutil --expand "$WORK/out.pkg" "$d"
  T="$WORK/target"; mkdir -p "$T"
  (cd "$T" && gzip -dc "$d/porthole-component.pkg/Payload" | cpio -id --quiet)
  (cd "$T" && gzip -dc "$d/mavergreen-base.pkg/Payload" | cpio -id --quiet)
  run sh "$d/porthole-component.pkg/Scripts/postinstall" "$WORK/out.pkg" "$T" "$T" "$T"
  [ "$status" -eq 0 ] || { echo "$output"; return 1; }
  [ -z "$(ls "$T/Applications" | grep -v '^Porthole.app$')" ]
}

@test "the tree's porthole is an exec shim into the engine, so the farm link keeps \$0 resolution" {
  sh "$ENGINE/packaging/macos/build_pkg.sh" 9.9.9 "$WORK/out.pkg" "$FAKEAPP" >/dev/null
  d="$WORK/expand"; pkgutil --expand "$WORK/out.pkg" "$d"
  T="$WORK/target"; mkdir -p "$T"
  (cd "$T" && gzip -dc "$d/porthole-component.pkg/Payload" | cpio -id --quiet)
  shim="$T/usr/local/mavergreen/porthole/bin/porthole"
  [ -x "$shim" ]
  [ ! -L "$shim" ]
  grep -qx 'exec "/Applications/Porthole.app/Contents/Resources/engine/bin/porthole" "$@"' "$shim"
}
